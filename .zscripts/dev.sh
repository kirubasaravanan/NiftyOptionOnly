...
# Start FastAPI backend (NIFTY engine API)
log_step_start "Starting FastAPI backend"
if [ -f "$PROJECT_DIR/scripts/supervisor.py" ]; then
    echo "[API] Starting nifty_engine FastAPI backend on port 8000..."
    cd "$PROJECT_DIR"
    python -u scripts/supervisor.py > "$PROJECT_DIR/.zscripts/api.log" 2>&1 &
    API_PID=$!
    echo "[API] Started supervisor (PID: $API_PID)"
    disown "$API_PID" 2>/dev/null || true
    
    # Wait for API to be ready
    api_attempt=1
    while [ "$api_attempt" -le 30 ]; do
        if curl -s --connect-timeout 2 --max-time 5 "http://localhost:8000/api/health" >/dev/null 2>&1; then
            echo "[API] FastAPI backend is ready on port 8000!"
            break
        fi
        echo "[API] Attempt $api_attempt/30: backend not ready yet, waiting..."
        sleep 2
        api_attempt=$((api_attempt + 1))
    done
    
    if [ "$api_attempt" -gt 30 ]; then
        echo "[API] WARNING: FastAPI backend did not start within 60s — dashboard will show errors"
    fi
else
    echo "[API] WARNING: scripts/supervisor.py not found — FastAPI backend will not start"
fi
log_step_end "Starting FastAPI backend"
